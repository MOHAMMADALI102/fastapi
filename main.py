...
def search_data(name: str):
    try:
        ...
        # Execute an SQL SELECT query to search for data in the "student" table
        ...
        if rows:
            ...
            return {"message": "data dont exist"}
    except psycopg2.Error as e:
        raise HTTPException(status_code=500, detail=f"Database error: {e}")
...
# def insert_fee(name: str, id: int):
#     try:
#         conn = psycopg2.connect(host=db_host, dbname=db_name, user=db_user, password=db_pass, port=db_port)
#         # Create a cursor object to execute SQL queries
#         cur = conn.cursor()
#
#         # Execute an SQL INSERT query to insert data into the "student" table
#         cur.execute("INSERT INTO student (name, id) VALUES (%s, %i)", (name, id))
#
#         # Commit the transaction
#         conn.commit()
#
...
# # Define a route to insert data into the database
# @app.post("/insert/")
# def insert_fee(name: str, city: str):
#     return insert_data(name, city)
